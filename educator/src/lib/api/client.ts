/**
 * Base API Client for educator UI
...
export class ApiClient {
	 ...
	 * - Client-side: Sends cookies automatically (credentials: 'include')
	 * - Server-side (SSR): Reads token from cookies, sends in Authorization header to Django
	 */
				...
				if (!isRefreshing) {
					...
					if (refreshResponse.ok) {
						...
						if (typeof window !== 'undefined') {
							localStorage.removeItem('educator_user'); // Clear user data
