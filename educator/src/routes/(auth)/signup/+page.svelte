<script lang="ts">
	...
	let userType = $state<'educator' | 'company' | null>(untrack(() => invitationToken ? 'company' : null));
	...
	let formData = $state({
		accountType: '' as 'educator' | 'company',
		companyName: '',
		industry: '',
		companySize: '',
		website: '',
	...
	function selectAccountType(type: 'educator' | 'company') {
		userType = type;
...
<svelte:head>
	<title>Sign Up - InaWorks educator</title>
		...
		<div class="space-y-8 lg:sticky lg:top-8 hidden lg:block">
			<div>
				...
				<p class="text-base xl:text-lg text-muted leading-relaxed">
					Access India's largest talent pool with 100k+ active job seekers. Post unlimited courses for free and find the perfect candidates faster.
				</p>
			</div>
				...
				<div class="space-y-5">
					<div class="flex items-start gap-4">
						...
						<div>
							...
							<p class="text-sm text-muted leading-relaxed">Post unlimited courses for free and get applications within hours</p>
						</div>
					</div>
					<div class="flex items-start gap-4">
		...
		<Card padding="lg" class="shadow-lg">
			...
			{:else}
				<form method="POST" action={isInvitationFlow ? '?/acceptInvitation' : '?/register'} use:enhance={({ cancel, formData: submitData }) => {
					const isFinalStep = (userType === 'company' && step === 3) || (userType === 'educator' && step === 2) || (isInvitationFlow && step === 2);
					if (!isFinalStep) {
						cancel();
						nextStep();
						return;
					...
					{#if step === 0 && !isInvitationFlow}
						...
						<div class="space-y-4">
							...
							<div class="grid grid-cols-1 gap-3">
									...
									<div class="flex items-start gap-4">
										...
										<div class="flex-1">
											...
											<p class="text-sm text-muted">
												I'm registering on behalf of a company to post courses and hire talent.
											</p>
											<ul class="mt-2 space-y-1 text-sm text-muted">
												<li class="flex items-center gap-2">
													<CheckCircle class="w-4 h-4 text-success" />
													Post unlimited courses
												</li>
												<li class="flex items-center gap-2">
													<CheckCircle class="w-4 h-4 text-success" />
													Manage team members
								...
								<button
									type="button"
									onclick={() => selectAccountType('educator')}
									class="p-5 border-2 border-border rounded-lg hover:border-primary hover:bg-primary/5 transition-all text-left group"
								>
									<div class="flex items-start gap-4">
										<div class="w-12 h-12 rounded-lg bg-primary/10 group-hover:bg-primary flex items-center justify-center transition-colors">
										...
										<div class="flex-1">
											<h4 class="text-lg font-semibold text-black mb-1">Independent educator</h4>
											<p class="text-sm text-muted">
												I'm an independent educator or consultant working with multiple companies.
											</p>
											<ul class="mt-2 space-y-1 text-sm text-muted">
												<li class="flex items-center gap-2">
													<CheckCircle class="w-4 h-4 text-success" />
													Post courses for clients
												</li>
												<li class="flex items-center gap-2">
													<CheckCircle class="w-4 h-4 text-success" />
													Manage candidates
					...
					{:else if (userType === 'company' && step === 2) || (userType === 'educator' && step === 1) || (isInvitationFlow && step === 1)}
							...
							{#if !isInvitationFlow}
								...
								<FormField label={userType === 'company' ? 'Your Job Title' : 'Professional Title'} required>
									<Input
										...
										required
										placeholder={userType === 'company' ? 'HR Manager, educator, etc.' : 'Senior educator, Talent Acquisition Specialist, etc.'}
										size="lg"
									/>
								</FormField>
							{/if}
						</div>
					{:else if (userType === 'company' && step === 3) || (userType === 'educator' && step === 2) || (isInvitationFlow && step === 2)}
					...
					{#if step > 0}
						...
						<div class="flex items-center justify-between mt-6 pt-6 border-t border-border">
							...
							{#if (userType === 'company' && step < 3) || (userType === 'educator' && step < 2) || (isInvitationFlow && step < 2)}
								<Button type="submit">
									Continue
								</Button>
							{:else}
