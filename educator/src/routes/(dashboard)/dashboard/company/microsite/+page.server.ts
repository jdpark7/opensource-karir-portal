...
export const load: PageServerLoad = async ({ parent, fetch }) => {
	...
	try {
		...
		const response = await fetch(`${API_BASE_URL}/educator/company/menu/`);
...
export const actions: Actions = {
	...
	createMenu: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/company/menu/add/`, {
				method: 'POST',
				headers: {
	...
	updateMenu: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/company/menu/edit/${menuId}/`, {
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	deleteMenu: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/company/menu/delete/${menuId}/`, {
				method: 'DELETE'
			});
	...
	toggleMenuStatus: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/company/menu/status/${menuId}/`, {
				method: 'POST'
			});
