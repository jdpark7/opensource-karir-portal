...
export const load: PageServerLoad = async ({ parent, fetch }) => {
	...
	try {
		const [teamResponse, invitationsResponse] = await Promise.all([
			fetch(`${API_BASE_URL}/educator/team/`),
			user.is_admin ? fetch(`${API_BASE_URL}/educator/team/invitations/`) : null
		]);
...
export const actions: Actions = {
	...
	inviteMember: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/team/invite/`, {
				method: 'POST',
				headers: {
	...
	updateMember: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/team/${userId}/update/`, {
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	removeMember: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/team/${userId}/remove/`, {
				method: 'DELETE'
			});
	...
	resendInvitation: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(
				`${API_BASE_URL}/educator/team/invitations/${invitationId}/resend/`,
				{
					method: 'POST'
				}
			);
	...
	cancelInvitation: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(
				`${API_BASE_URL}/educator/team/invitations/${invitationId}/cancel/`,
				{
					method: 'DELETE'
				}
			);
	...
	toggleStatus: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`${API_BASE_URL}/educator/team/${userId}/toggle-status/`, {
				method: 'POST'
			});
