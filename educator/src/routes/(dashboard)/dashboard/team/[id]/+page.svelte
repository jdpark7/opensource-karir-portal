...
<div class="space-y-6">
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
		<div class="bg-white rounded-lg border border-border p-6">
			<div class="flex items-center gap-3 mb-3">
				...
				<h3 class="text-sm font-medium text-muted">Courses Posted</h3>
			</div>
			<div class="text-3xl font-bold text-black">{member.stats?.courses_posted || 0}</div>
			<div class="flex items-center gap-1 mt-2 text-sm text-muted">
				<TrendingUp class="w-4 h-4" />
				<span>{member.stats?.active_courses || 0} currently active</span>
			</div>
		</div>
		...
		<div class="bg-white rounded-lg border border-border p-6">
			...
			<div class="text-sm text-muted mt-2">Across all courses</div>
		</div>
	...
	<!-- Recent Courses -->
	{#if member.recent_courses && member.recent_courses.length > 0}
		<div class="bg-white rounded-lg border border-border">
			...
			<div class="divide-y divide-border">
				{#each member.recent_courses as job}
					<a
	...
	{:else}
		<div class="bg-white rounded-lg border border-border p-12 text-center">
			...
			<h3 class="text-lg font-semibold text-black mb-2">No courses posted yet</h3>
			<p class="text-muted">This team member hasn't posted any courses</p>
		</div>
	{/if}
	...
	<div class="bg-white rounded-lg border border-border p-6">
		...
		<div class="space-y-2">
			<div class="flex items-center justify-between py-2">
				<span class="text-muted">Post Courses</span>
				{#if member.permissions?.can_post_courses}
