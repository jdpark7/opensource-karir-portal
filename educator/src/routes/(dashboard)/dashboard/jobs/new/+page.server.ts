...
export const load: PageServerLoad = async ({ cookies, fetch, url }) => {
	...
	try {
		...
		const apiUrl = 'http://localhost:8000/api/v1/educator/courses/metadata/';
		...
		if (copyFromJobId) {
			try {
				const jobResponse = await fetch(`http://localhost:8000/api/v1/educator/courses/${copyFromJobId}/`);
...
export const actions: Actions = {
	...
	saveDraft: async ({ request, cookies, fetch }) => {
		...
		try {
			...
			const response = await fetch('http://localhost:8000/api/v1/educator/courses/create/', {
				method: 'POST',
				headers: {
	...
	publish: async ({ request, cookies, fetch }) => {
		...
		try {
			...
			const createResponse = await fetch('http://localhost:8000/api/v1/educator/courses/create/', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
