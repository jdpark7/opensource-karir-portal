/**
 * Inactive Courses Page - Server Load
 * Shows Disabled and Expired courses in a dedicated view
 */
import type { PageServerLoad, Actions } from './$types';
import { error, redirect } from '@sveltejs/kit';
import type { CoursesListResponse } from '$lib/types';
...
export const load: PageServerLoad = async ({ fetch, url, cookies }) => {
	...
	try {
		...
		// Filter for inactive courses only (Disabled OR Expired)
		...
		const apiUrl = `http://localhost:8000/api/v1/educator/courses/?${params.toString()}`;
		...
		if (!response.ok) {
			...
			throw error(response.status, `Failed to load inactive courses: ${response.statusText}`);
		}

		const data: CoursesListResponse = await response.json();

		// Filter to show only inactive courses (Disabled or Expired)
		let filteredCourses = data.results;
		if (statusFilter === 'all' || !statusFilter) {
			// When showing all inactive, filter to only Disabled and Expired
			filteredCourses = data.results.filter(
				(job) => job.status === 'Disabled' || job.status === 'Expired'
			);
		}

		// Get counts for each status from all fetched courses
		...
		console.error('Error loading inactive courses:', err);
		...
		throw error(500, err.message || 'Failed to load inactive courses');
	}
};
...
export const actions: Actions = {
	...
	reactivate: async ({ request, fetch, cookies }) => {
		...
		try {
			const response = await fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/update/`, {
				method: 'PATCH',
				headers: {
	...
	delete: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(
				`http://localhost:8000/api/v1/educator/courses/${jobId}/delete/?force=true`,
				{
					method: 'DELETE'
				}
			);
