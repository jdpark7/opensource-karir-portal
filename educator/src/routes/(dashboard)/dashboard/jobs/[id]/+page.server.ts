...
export const load: PageServerLoad = async ({ params, cookies, fetch }) => {
	...
	try {
		// Fetch job details
		const jobResponse = await fetch(`${API_BASE_URL}/educator/courses/${jobId}/`, {
			headers: {
		...
		const applicantsResponse = await fetch(`${API_BASE_URL}/educator/courses/${jobId}/applicants/`, {
			headers: {
				Authorization: `Bearer ${accessToken}`
			}
		});
		...
		const analyticsResponse = await fetch(`${API_BASE_URL}/educator/courses/${jobId}/analytics/?period=30d`, {
			headers: {
				Authorization: `Bearer ${accessToken}`
			}
		});
...
export const actions: Actions = {
	...
	toggleNotifications: async ({ params, cookies, fetch }) => {
		...
		try {
			...
			// Update the job
			const updateResponse = await fetch(`${API_BASE_URL}/educator/courses/${jobId}/update/`, {
				method: 'PATCH',
				headers: {
