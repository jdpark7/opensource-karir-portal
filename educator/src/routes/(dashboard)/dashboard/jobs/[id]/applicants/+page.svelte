<script lang="ts">
	...
	import ApplicantDetailModal from '$lib/components/educator/ApplicantDetailModal.svelte';
	...
	async function viewApplicantDetail(applicantId: number) {
		...
		try {
			const response = await fetch(
				`${API_BASE_URL}/educator/courses/${data.job.id}/applicants/${applicantId}/`,
				{
					headers: {
...
<div class="space-y-6">
	...
	<div class="bg-white rounded-lg border border-border p-4">
		<div class="flex flex-col md:flex-row gap-4">
			...
			<!-- Download CSV Button -->
			<a
				href="/dashboard/courses/{data.job.id}/applicants/download?status={selectedFilter !== 'all' ? selectedFilter : ''}&search={searchQuery}"
