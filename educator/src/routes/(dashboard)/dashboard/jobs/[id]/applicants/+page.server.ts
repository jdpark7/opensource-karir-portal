...
export const load: PageServerLoad = async ({ params, url, cookies, fetch }) => {
	...
	try {
		...
		const [jobResponse, applicantsResponse] = await Promise.all([
			fetch(`${API_BASE_URL}/educator/courses/${jobId}/`, {
				headers: {
					Authorization: `Bearer ${accessToken}`
				}
			}),
			fetch(`${API_BASE_URL}/educator/courses/${jobId}/applicants/?${queryParams.toString()}`, {
				headers: {
					Authorization: `Bearer ${accessToken}`
				}
			})
	...
	updateStatus: async ({ request, params, cookies, fetch }) => {
		...
		try {
			const response = await fetch(
				`${API_BASE_URL}/educator/courses/${jobId}/applicants/${applicantId}/update/`,
				{
					method: 'PATCH',
					headers: {
