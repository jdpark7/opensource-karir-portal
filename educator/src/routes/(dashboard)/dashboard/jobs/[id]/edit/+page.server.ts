 ...
 *
 * This file handles server-side rendering and form submission for editing existing courses.
...
export const load: PageServerLoad = async ({ params, cookies, fetch, url }) => {
	...
	try {
		...
		const [jobResponse, metadataResponse] = await Promise.all([
			fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/`),
			fetch('http://localhost:8000/api/v1/educator/courses/metadata/')
		]);
		...
		// Prevent editing published courses - redirect to courses list
		if (job.status === 'Live' || job.status === 'Disabled' || job.status === 'Expired') {
			throw redirect(302, '/dashboard/courses/?error=' + encodeURIComponent(`Cannot edit a job with status '${job.status}'. Only Draft courses can be edited.`));
		}
...
export const actions: Actions = {
	...
	publish: async ({ params, request, cookies, fetch }) => {
		...
		try {
			...
			// Step 1: Update job
			const updateResponse = await fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/update/`, {
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json'
				},
			...
			if (updateResult.job.status !== 'Live') {
				const publishResponse = await fetch(
					`http://localhost:8000/api/v1/educator/courses/${jobId}/publish/`,
					{
						method: 'POST',
						headers: {
							'Content-Type': 'application/json'
