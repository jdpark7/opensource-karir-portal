/**
 * Server-side load function for courses page
...
export const load: PageServerLoad = async ({ fetch, url, cookies }) => {
	...
	try {
		...
		if (!response.ok) {
			...
			throw error(response.status, `Failed to load courses: ${response.statusText}`);
		}
...
export const actions: Actions = {
	...
	publish: async ({ request, fetch, cookies }) => {
		...
		try {
			const response = await fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/publish/`, {
				method: 'POST',
				headers: {
	...
	close: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/close/`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				}
	...
	delete: async ({ request, fetch }) => {
		...
		try {
			const response = await fetch(`http://localhost:8000/api/v1/educator/courses/${jobId}/delete/?force=true`, {
				method: 'DELETE'
			});
