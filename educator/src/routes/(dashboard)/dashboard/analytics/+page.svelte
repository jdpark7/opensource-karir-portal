<script lang="ts">
	...
	$: overviewMetrics = [
		...
		},
		{
			...
			value: analytics.overview.total_courses.toString(),
...
<svelte:head>
	<title>Analytics - InaWorks educator</title>
</svelte:head>
	...
	<div class="bg-white rounded-lg border border-border">
					...
					<tbody class="divide-y divide-border">
						{#each analytics.job_performance as job}
							<tr class="hover:bg-surface">
								<td class="px-6 py-4">
									<a href="/dashboard/courses/{job.job_id}/" class="text-sm font-medium text-primary hover:text-primary">
										{job.job_title}
									</a>
								</td>
		...
		{:else}
			<div class="p-12 text-center">
				...
				<p class="text-muted">No active courses in this period</p>
			</div>
		{/if}
	</div>
