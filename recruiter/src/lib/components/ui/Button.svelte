<script lang="ts">
	...
	// Create a union type for props that can be either for a button or an anchor
...
{#if 'href' in restProps && restProps.href}
	<a class={classes} href={restProps.href as string} {...restProps as HTMLAnchorAttributes}>
		{@render children()}
	</a>
{:else}
	<button class={classes} disabled={disabled || loading} type={restProps.type as "button" | "submit" | "reset" || 'button'} {...restProps as HTMLButtonAttributes}>
		{#if loading}
			<svg
