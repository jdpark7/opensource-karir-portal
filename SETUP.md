...
DATABASE_URL=postgresql://postgres:password@localhost/inaworks
REDIS_URL=redis://localhost:6379/0
ELASTICSEARCH_URL=http://localhost:9200
PEEL_URL=http://localhost:8000/
DEFAULT_FROM_EMAIL=noreply@inaworks.local
```
...
```bash
# Create database
sudo -u postgres createdb inaworks
...
DATABASE_URL=postgresql://inaworks_user:password@localhost/inaworks_prod
...
**Django Service** (`/etc/systemd/system/inaworks.service`):

```ini
[Unit]
Description=InaWorks Django Application
After=network.target

[Service]
User=www-data
WorkingDirectory=/var/www/inaworks
Environment="PATH=/var/www/inaworks/venv/bin"
ExecStart=/var/www/inaworks/venv/bin/gunicorn --workers 3 --bind unix:/run/inaworks/inaworks.sock jobsp.wsgi:application
Restart=on-failure
...
**Celery Service** (`/etc/systemd/system/inaworks-celery.service`):

```ini
[Unit]
Description=InaWorks Celery Worker
After=network.target
...
WorkingDirectory=/var/www/inaworks
Environment="PATH=/var/www/inaworks/venv/bin"
ExecStart=/var/www/inaworks/venv/bin/celery -A jobsp worker --loglevel=info
Restart=on-failure
...
server {
    ...
    location / {
        ...
        proxy_pass http://unix:/run/inaworks/inaworks.sock;
    }
}
```
